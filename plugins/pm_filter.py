...
async def pm_spoll_choker(msg):
    ...
    gs_parsed = []
    if not g_s:
        k = await msg.reply("I couldn't find any 𝙰𝙽𝙸𝙼𝙴 in that name.")
    ...
    if not movielist:
        k = await msg.reply("I couldn't find any anime related to that. Check your spelling")
        await asyncio.sleep(8)
        await k.delete()
        return
    ...
    await msg.reply("I couldn't find any anime related to that\nDid you mean any one of these?\nThen Spiderman Will Give You That", reply_markup=InlineKeyboardMarkup(btn), reply_to_message_id=msg.id)
